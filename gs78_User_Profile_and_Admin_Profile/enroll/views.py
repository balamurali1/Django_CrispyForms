...
def user_profile(request):
	if request.user.is_authenticated:  #is_authenticated ante credentials(username,Password) unte ne profile page kanapadali ani artham..
		...
		else:  #ee else anedi method=="GET" ayyenappudu execute auvthundi..
			if  request.user.is_superuser == True:
				fm=EditAdminForm(instance=request.user)
				users = User.objects.all()
			else:		
				fm = EditUserProfileForm(instance=request.user) #request.user--> ante login aina person ani artham.. 
				users = None
